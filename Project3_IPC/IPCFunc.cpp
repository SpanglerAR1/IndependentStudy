...
int 	Init_Socket()
{
	...
	while (1)
	{
		...
		waitingconnections = connectionset;
		if (select(FD_SETSIZE, &waitingconnections, NULL, NULL, NULL) < 0)
		{
		...
		{
			if(FD_ISSET(i,&waitingconnections))
			{
				...
				if (i == sock)
				{
					...
					FD_SET(receivesocket,&connectionset);
				}
				else
				{
					...
					{
						close(i);
						printf("Reached EOF on filedes %d\n",i);
						FD_CLR(i,&connectionset);
					}
				}
			}
		}
